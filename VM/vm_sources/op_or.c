...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/29 16:02:12 by dhuber            #+#    #+#             */
/*   Updated: 2018/09/25 17:59:04 by dhuber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check(t_vm *vm, t_player *player, int *arg)
{
	...
	{
		if (vm->t == 1)
			ft_printf("%s failed to execute or at cycle [%d].\n",
...
int				op_or(t_vm *vm, t_player *player, int *arg)
{
	...
	move_pc(vm, player);
	if (vm->t == 1)
		ft_printf("%s executes successfully or at cycle [%d].\n",
		player->name, vm->cycle);
	return (OP_SUCCESS);
}
