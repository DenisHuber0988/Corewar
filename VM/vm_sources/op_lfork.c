...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/29 16:06:15 by dhuber            #+#    #+#             */
/*   Updated: 2018/09/25 18:00:39 by dhuber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				op_lfork(t_vm *vm, t_player *player, int *arg)
{
	short	adr;

	arg[0] = recup(vm, player, (player->address + player->pc + 1) % MEM_SIZE);
	adr = (player->pc + arg[0]) % MEM_SIZE;
	add_fork(vm, player, adr);
	vm->nbr_fork += 1;
	move_pc(vm, player);
	if (vm->t == 1)
		ft_printf("%s executes successfully a lfork at cycle [%d].\n",
