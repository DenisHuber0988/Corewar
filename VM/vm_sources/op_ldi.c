...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/29 15:33:19 by dhuber            #+#    #+#             */
/*   Updated: 2018/09/25 18:00:34 by dhuber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check(t_vm *vm, t_player *player, int *arg)
{
	if ((arg[0] == -1 && player->type[0] == REG_CODE) || \
			...
			|| player->type[0] == -1 || player->type[1] == -1 \
			|| player->type[2] != REG_CODE || player->type[3])
	{
		if (vm->t == 1)
			ft_printf("%s failed to execute ldi at cycle [%d].\n",
...
int				op_ldi(t_vm *vm, t_player *player, int *arg)
{
	int		adr;
