...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/29 15:38:43 by dhuber            #+#    #+#             */
/*   Updated: 2018/09/25 18:01:11 by dhuber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check(t_vm *vm, t_player *player, int *arg)
{
	...
	if ((player->type[0] == REG_CODE && arg[0] == -1) || player->type[0] == -1 \
			|| (player->type[1] == REG_CODE && arg[1] == -1) \
			|| player->type[1] == -1 || player->type[2] == -1 \
			|| player->type[3])
		i++;
	if (arg[2] == -1 || player->type[2] != REG_CODE || i)
	{
		if (vm->t == 1)
			ft_printf("%s failed to execute xor at cycle [%d].\n",
...
int				op_xor(t_vm *vm, t_player *player, int *arg)
{
	...
	move_pc(vm, player);
	if (vm->t == 1)
		ft_printf("%s executes successfully xor at cycle [%d].\n",
		player->name, vm->cycle);
	return (OP_SUCCESS);
}
